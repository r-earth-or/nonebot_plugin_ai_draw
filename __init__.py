...
async def draw_group(bot: Bot, event: GroupMessageEvent):
    ...
    if len(user_data) == 0:
        # 注册个人使用次数限制
        logger.debug(4)
        user_data_cur.execute('''insert into USER values(?,?,?,?)''', (userid, 5, 5, int(time.time())))
        user_data_con.commit()
        user_data = [(userid, 5, 5, int(time.time()))]
    ...
    url = "http://91.217.139.190:5010/got_image?tags=" + text + f"&r18=0&token={token}"
    async with httpx.AsyncClient() as client:
